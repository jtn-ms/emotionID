...
class FullScreenApp(object):
    ...
    def Space_Pressed(self,event):
        	#
        	if  self.Stages[self.currentStageID] == 'Preparing' and \
        	    self.CorrectPosed:
    ...
    def show_outputframe(self):
        if (self.featurepoints == 0).sum() == self.featurepoints.shape[0] * self.featurepoints.shape[1]:
            return
    ...
    def ready4Training(self,img,shape,faceindex,x,y):
        	...
        	rotated,tilted = 0,0
        	for i in range(6):#using two eyes
    ...
    def show_landmark(self,img):
        ...
        for (i, rect) in enumerate(rects):
        	    ...
        	    self.ready4Training(img,coords,i,x,y)		    
        	    #return shape
